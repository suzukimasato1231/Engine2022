...
void Object::NoShadowMatWorld(ObjectData& polygon, PSR& psr, Vec3 position, Vec3 scale, Vec3 rotation, Vec4 color)
{
	HRESULT result;
	if (psr.position.x != position.x || psr.position.y != position.y || psr.position.z != position.z
		|| psr.scale.x != scale.x || psr.scale.y != scale.y || psr.scale.z != scale.z
...
void Object::NoShadowDraw(ObjectData& polygon, PSR& psr, Vec3 position, Vec3 scale, Vec3 rotation, Vec4 color, int graph)
{
	if (OBJNum >= Object::OBJbuffer.size())
	{
		//定数バッファ
	...
	//更新
	NoShadowMatWorld(polygon,psr,position, scale, rotation, color);
